...
the maintained pull request that adds it.
...
**Canvas:** only a multiple of 32 is allowed. A smaller canvas is much faster
per step — `544×960` runs ~2.3× faster than the trained `768p` short edge.
...
## 5. Load the model (int8, Quantization)

Both big components (the 61.7 GB transformer and the 62.1 GB Qwen3-VL
conditioner) are loaded as **int8 weight-only quantized** — this brings them
down to ~62 GB combined so they fit on a 96 GB card. We then perform the setup
steps (`requires_grad_(False)`) and **disable gradient tracking** to keep memory
low and inference fast.
...
This is the slow step (a 30-step 544×960 clip takes roughly 2–5 min on a 96 GB
Blackwell). The model returns the video frames **and** its own stereo
soundtrack together. We wrap generation so any error prints clearly instead of
